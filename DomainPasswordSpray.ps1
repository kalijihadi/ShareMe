<#
this code was copied form https://github.com/dafthack/DomainPasswordSpray and changed due to a bug causing domain wide user lockouts

# Originals Script Bug Explained
The bug is caused when the script doesn’t manage to query the domains lockout observation window in the Get-ObservationWindow function.
this causes $observation_window to get a null value.
When the script removes the users that are on the lockout threshold the following condition appears:

if (($timedifference -gt $observation_window) -or ($attemptsuntillockout -gt 1))

As you can see, when the value of $observation_window is null this condition is always true, thus adding the users that are on the lockout threshold to the list of users to spray.
Take note that the $observation_window only plays a part when the scripts is used with the parameter -PasswordList, but the $observation_window is always queried and receives the value null even when a password list isn’t given.

# The Fix
Replacing the problematic line with the following line:

if ($attemptsuntillockout -gt 2)

This removes any option to “bypass” the threshold check, notice that the value of $attemptsuntillockout was raised to two tries instead of one, of extra safety precautions.
Also take note that the "cannot index to null array" error that accrued in the case of an unsuccessful query of the lockout observation window, was suppressed.

Important Note:
1. It is advised not to use the -Passwordlist option, because after this fix there will be no time gap between the different passwords that are sprayed (unlike what was intended in the original script).
the passwords will keep getting sprayed on every user until the user is two attempts from the lockout threshold (even then, no user lockout is expected).
    
2. When executing with -Userlist option, take not that the script DOESN’T CHECK for user lockouts  (this is also the case in the original script).
#>
...
<#
    ...
    {
        Write-Host -ForegroundColor "yellow" "[*] Removing users within 2 attempt of locking out from list."
        foreach ($user in $AllUserObjects)
        {
            ...
            if ($badcount)
            {
                ...
                # original condition - casess lockout because $observation_window is null
                # if (($timedifference -gt $observation_window) -or ($attemptsuntillockout -gt 1))

                # new condition - no bypass to attempts until lockout, raised to 2 just in case
                if ($attemptsuntillockout -gt 2)
                                {
...
function Get-ObservationWindow($DomainEntry)
{
    ...
    # edited to suppress ovservation window error
    $observation_window = @()
    try{
